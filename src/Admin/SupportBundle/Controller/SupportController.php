...
class SupportController extends Controller
{
     ...
     * @return array
     *
     * @Route("/edit-reply/{id}", name="support_edit_reply")
     * @Template("AdminSupportBundle::edit_reply.html.twig")
     */
    public function editReplyAction(Request $request, $id)
    {
        $em = $this->getDoctrine()->getManager();
        $reply = $em->getRepository('SupportBundle:ReplySupportTicket')->find($id);
        ...
        if ($request->isMethod('POST')) {
            $form->handleRequest($request);
            if ($form->isValid()) {
                /** @var $data ReplySupportTicket */
                $data = $form->getData();

                $em->flush();

                $this->addFlash('success', $this->get('translator')->trans('saved'));

                return $this->redirectToRoute('support_reply_ticket', array('id' => $data->getTicket()->getId()));
            }
        }
