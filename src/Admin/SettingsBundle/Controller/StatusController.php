...
//        $statuses = $em->getRepository('AdminSettingsBundle:MemberStatus')->findBy(array(), array('id' => 'ASC'));
        ...
        $grid->getColumn('price')->setTitle('Цена')->setSize(50);
        $grid->getColumn('percent')->setTitle('Процент от баллов (%)')->setSize(150);
        $grid->getColumn('countTokens')->setTitle('Токены')->setSize(100);
        $grid->getColumn('scaleLevels')->setTitle('Уровни линейки')->setSize(150);
        $grid->getColumn('maxBinaryProfit')->setTitle('Макс. прибыль с бинара')->setSize(150);
